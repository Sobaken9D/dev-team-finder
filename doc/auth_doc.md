...
4. "@auth/prisma-adapter" — это связующее звено между библиотекой аутентификации
   Auth.js и вашей базой данных через ORM Prisma. Делает автоматическую
   синхронизацию данных пользователя между БД и библиотекой аутентификации.
...
В файле (/proxy.ts) перечисляется набор защищенных роутов. Т.е ссылки на
